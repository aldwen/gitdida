from dida365api.models.attachment import Attachment
...
from dida365api.utils.time_util import get_prc_arrow, get_today_arrow, get_utc_str
...
class Task:
    ...
    def _load_field(self):
        ...
        self.url = Link.LINK_TEMPLATE.format(
            project_id=self.project_id, task_id=self.id
        )
    ...
    def change_start_date_to_today(self):
        ...
        self.start_date = self.start_date.replace(
            year=ta.year, month=ta.month, day=ta.day
        )
        self.task_dict[Task.START_DATE] = get_utc_str(self.start_date)
