...
class TaskSelector:
    ...
    def _earliest_start_date(self):
        return sorted(self.target_tasks, key=lambda x: x.created_time)[
            : self.reallocation_len
        ]

    def _early_group_round_robin(self):
        tasks = sorted(self.target_tasks, key=lambda x: x.created_time)
        create_time_task_mapping = [
            [i[0], list(i[1])]
            for i in groupby(
                tasks, lambda x: x.created_time.replace(hour=0, minute=0, second=0)
            )
        ]
        task_group = [
            i
            for i in create_time_task_mapping
            if get_days_offset(get_today_arrow(), i[0]) > 3
        ]
