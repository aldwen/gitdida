...
from dida365api.api.dida365 import Dida365
from dida365api.exceptions.backlink_exceptions import TaskNotFoundException
from dida365api.models.backlink import BackLink
from dida365api.models.link import Link
from dida365api.models.target_date import TargetDate
from dida365api.models.task import Task
from dida365api.models.upload_attachment import uploadAttachment
from dida365api.utils.backlink_util import BackLinkUtil
from dida365api.utils.commom_util import groupby_func
from dida365api.utils.decorator_util import ensure_run_retry
from dida365api.utils.dict_util import BaiduFanyi
from dida365api.utils.dictvoice_util import get_dictvoice_bytes
...
class DidaManipulate:
    ...
    def _get_target_words_task(self, start_day_offset):
        def condition(task: Task):
            return (
                ...
                and get_days_offset(task.start_date, self.today_arrow)
                == start_day_offset
            )

        tasks = filter(
            lambda task: re.search(
                r".*" + DidaManipulate.PROJECT_WORDS.decode("utf-8") + r"$",
    ...
    def reallocate_task(self, start_day_offset, selector):
        ...
        if task_len > self.quantity_limit:
            print(
                f"Task quantity is {task_len}, quantity_limit is {self.quantity_limit}, will reallocate {reallocation_len} tasks.\n"
            )
            ...
            for task in task_selector.select_task(selector):
                ...
                print(
                    f"Change start_date from [{task.org_start_date}] to [{task.start_date}], task created_time is [{task.created_time}], title is [{task.title}]"
                )
    ...
    def build_backlink(self):
        for task in [
            i
            for i in self.dida.active_tasks
            if i.project_id == "670946db840bf3f353ab7738"
        ]:
            normal_links = Link.dedup_link_with_wls(
                task._backlink_util.parse_normal_links()
            )
            for normal_link in normal_links:
                target_tasks = [
                    i
                    for i in self.dida.active_tasks
                    if i.id == normal_link.link_task_id
                ]
                ...
                if len(target_tasks) > 1:
                    raise UserWarning(
                        f"Task id duplicates, id: {normal_link.link_task_id}"
                    )
                ...
                if backlink not in target_task_backlinks:
                    ...
                    backlink = target_task_backlinks[
                        target_task_backlinks.index(backlink)
                    ]
                    backlink.add_whole_line_str(normal_link.whole_line_str)
                backlink_section_str = BackLinkUtil.gen_backlink_section(
                    target_task_backlinks
                )
                if if_add_section:
                    ...
                    content = "" if target_task.content is None else target_task.content
                    content = re.sub(
                        BackLinkUtil.SECTION_PATTERN, backlink_section_str, content
                    )
                if target_task.content != content:
                    target_task.update_content(content)
                    self.dida.post_task(
                        Task.gen_update_date_payload(target_task.task_dict)
                    )
                    print(
                        f'{"Create" if if_add_section else "Update"} backlink: [{target_task.title}] <- [{task.title}]'
                    )
    ...
    def find_task(self, task_title, if_reload_data=False):
        ...
        if len(tasks) != 1:
            raise UserWarning(
                f"Task with title[{task_title}] duplicated, count: {len(tasks)}"
            )
        return tasks[0]
    ...
    def add_dictvoice_existing_task(self):
        ...
        task.add_upload_attachment_post_payload_by_bytes(
            *get_dictvoice_bytes(query_word)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-n",
        "--new",
        help="Add single new ebbinghaus task with the word specific",
        type=str,
    )
    parser.add_argument(
        "-f",
        "--new_from_file",
        help="Add multiple new ebbinghaus tasks using a words.txt file at given path.",
        action="store_true",
    )
    parser.add_argument(
        "-b", "--backlink", help="If build backlink", action="store_true"
    )
    parser.add_argument(
        "-p", "--perpetuate", help="If perpetuate completed tasks.", action="store_true"
    )
    parser.add_argument(
        "-r", "--reallocate", help="If reallocate tasks.", action="store_true"
    )
    parser.add_argument(
        "-o", "--renew", help="If renew overdue tasks.", action="store_true"
    )
    parser.add_argument(
        "-a", "--add_dictvoice", help="Add dictvoice to existing task.", type=str
    )
    parser.add_argument(
        "-d",
        "--default",
        help="If run default procedure: 1.perpetuate_task 2.build_backlink 3.reallocate_task.",
        action="store_true",
    )
    parser.add_argument(
        "--full_scan_closed_task",
        help="Scan all closed tasks, otherwise only within 7 days.",
        action="store_true",
    )
    parser.add_argument(
        "--start_day_offset",
    ...
    parser.add_argument(
        "--quantity_limit",
        help="Quantity limit for reallocation task",
        default=20,
        type=int,
    )
    parser.add_argument(
