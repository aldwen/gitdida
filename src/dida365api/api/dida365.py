...
from dida365api.models.project import Project
...
from dida365api.utils.time_util import get_standard_str, get_today_arrow
...
class Dida365:
    ...
    def get_closed_task(self, project_id="all"):
        ...
        while True:
            ...
            print(
                f"Search completed task:[{get_standard_str(datetime_from)}]->[{get_standard_str(datetime_to)}], quantity:{len(data)}"
            )
            if len(data) == 50:
                for hour in range(24):
                    ...
                    data = request_data(datetime_from, datetime_to)
                    print(
                        f"Search by hour:[{get_standard_str(datetime_from)}]->[{get_standard_str(datetime_to)}], ，quantity:{len(data)}"
                    )
                    tasks.extend(data)
            else:
                tasks.extend(data)
            ...
            if self.quick_scan_closed_task:
                if (
                    date.year == deadline_date.year
                    and date.month == deadline_date.month
                    and date.day == deadline_date.day
                ):
                    break
            else:
    ...
    def upload_attachment(self, *attachments: uploadAttachment):
        for attachment in attachments:
            ...
            if attachment.file_bytes is not None:
                ...
                raise UserWarning(
                    f"Attachment without neither file bytes nor file path!"
                )
